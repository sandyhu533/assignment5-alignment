...
def compute_group_normalized_rewards(
    ...
    grouped_rewards = raw_rewards.reshape(rollout_batch_size//group_size, group_size) # (group_num, group_size)
    baselines = 0
    if baseline == "mean":
        baselines = grouped_rewards.mean(dim=-1, keepdim=True)
    normalizers = 1
    if advantage_normalizer == "std":
        normalizers = grouped_rewards.std(dim=-1, keepdim=True) + advantage_eps
    elif advantage_normalizer == "mean":
        normalizers = grouped_rewards.mean(dim=-1, keepdim=True) + advantage_eps
    grouped_advantages = (grouped_rewards - baselines)/normalizers
    advantages = grouped_advantages.reshape(rollout_batch_size)
    metadata = {
...
def compute_policy_gradient_loss(
    ...
    if importance_reweighting_method == "none":
        ...
        metadata = {}
        return -raw_rewards_or_advantages*reweight, metadata
    elif importance_reweighting_method == "grpo":
        reweight = (policy_log_probs-old_log_probs).exp()
        clipped = reweight.clip(1-cliprange, 1+cliprange)
        per_token_loss = torch.minimum(raw_rewards_or_advantages*reweight, raw_rewards_or_advantages*clipped)
        metadata = {}
        ...
        clipped = reweight.clip(1-cliprange, 1+cliprange)
        per_token_loss = torch.minimum(raw_rewards_or_advantages*reweight, 
                                       raw_rewards_or_advantages*clipped).expand_as(log_ratio)
        metadata = {}
        return -per_token_loss, metadata
